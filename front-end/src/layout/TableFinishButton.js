...
function TableFinishButton({status,table,loadDashboard}) {
 ...
 async function handleFinish(table_id,reservation_id){
    ...
    async function handleClick(){
        ...
        )? await handleFinish(table.table_id, table.reservation_id): null;
    }
  ...
  return (  status === "Occupied" && (
        ...
        <button    data-data-id-finish={table.table_id} type="button" className='btn btn-sm btn-primary' onClick={handleClick}>
            Finish
        </button>
